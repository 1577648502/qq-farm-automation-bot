  ...
  <div class="space-y-4">
    <div class="rounded-lg card">
      ...
      <div class="flex flex-col items-center justify-between gap-3 sm:gap-4 border-b border-gray-100 p-3 sm:p-4 sm:flex-row dark:border-gray-700">
        ...
        <div class="flex flex-nowrap gap-2 overflow-x-auto pb-1 scrollbar-none snap-x snap-mandatory sm:flex-wrap sm:overflow-visible sm:pb-0">
          <button
            v-for="op in operations"
            :key="op.type"
            class="min-h-[44px] flex items-center justify-center gap-1.5 rounded px-3 py-2 text-sm text-white transition whitespace-nowrap snap-start disabled:cursor-not-allowed disabled:opacity-50"
            :class="op.color"
            :disabled="operating"
      ...
      <div class="flex flex-wrap gap-2 sm:gap-4 border-b border-gray-100 bg-gray-50 p-3 sm:p-4 text-xs sm:text-sm dark:border-gray-700 dark:bg-gray-900/50">
        <div class="flex items-center gap-1 rounded-full bg-orange-100 px-2 sm:px-3 py-1 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">
          <div class="i-carbon-clean" />
          <span class="font-medium">可收: {{ summary?.harvestable || 0 }}</span>
        </div>
        <div class="flex items-center gap-1 rounded-full bg-green-100 px-2 sm:px-3 py-1 text-green-700 dark:bg-green-900/30 dark:text-green-400">
          <div class="i-carbon-sprout" />
          <span class="font-medium">生长: {{ summary?.growing || 0 }}</span>
        </div>
        <div class="flex items-center gap-1 rounded-full bg-gray-100 px-2 sm:px-3 py-1 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
          <div class="i-carbon-checkbox" />
          <span class="font-medium">空闲: {{ summary?.empty || 0 }}</span>
        </div>
        <div class="flex items-center gap-1 rounded-full bg-red-100 px-2 sm:px-3 py-1 text-red-700 dark:bg-red-900/30 dark:text-red-400">
      ...
      <div class="p-3 sm:p-4">
        ...
        <div v-else class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-3">
          <LandCard
            v-for="land in lands"
            :key="land.id"
            :land="land"
