...
function isIdleKickoutReason(reason = '') {
    const text = String(reason || '');
    return text.includes('长时间未操作') || text.includes('断开链接');
}

function isHeartbeatRequest(serviceName, methodName) {
    return serviceName === 'gamepb.userpb.UserService' && methodName === 'Heartbeat';
}
...
function sendMsgAsync(serviceName, methodName, bodyBytes, timeout = 20000) {
    return new Promise((resolve, reject) => {
        ...
        // 如果待处理请求过多，拒绝新请求避免堆积
        // 心跳是保活请求，允许它越过普通业务队列限制；否则业务请求卡住时，
        // 心跳会被挤掉，服务端可能按“长时间未操作”断开连接。
        const heartbeat = isHeartbeatRequest(serviceName, methodName);
        const pendingLimit = heartbeat ? 8 : 5;
        if (pendingCallbacks.size >= pendingLimit) {
...
function handleNotify(msg) {
    ...
    try {
        ...
        if (type.includes('Kickout')) {
            ...
            if (isIdleKickoutReason(reason)) {
                log('推送', `连接因长时间未操作断开: ${type}`);
            } else {
                log('推送', `被踢下线! ${type}`);
            }
            log('推送', `原因: ${reason}`);
            networkEvents.emit('kickout', {
                type,
                reason,
                idle: isIdleKickoutReason(reason),
            });
            return;
        }
    ...
    await sendMsg('gamepb.userpb.UserService', 'Login', body, (err, bodyBytes, _meta) => {
        ...
        try {
            ...
            if (reply.basic) {
                ...
                userState.diamond = toNum(reply.basic.diamond);
                ...
                // 登录后尝试通过 savedCode 查询 QQ 号 (uin)
                if (savedCode && !userState.uin) {
                    const { MiniProgramLoginSession } = require('../services/qrlogin');
                    MiniProgramLoginSession.queryStatus(savedCode).then((mpResult) => {
                        if (mpResult.status === 'OK' && mpResult.uin) {
                            userState.uin = String(mpResult.uin);
                            log('系统', `查询到 QQ 号: ${mpResult.uin}`);
                        }
                    }).catch(() => {
                        // 忽略查询 uin 失败，不影响主流程
                    });
...
const HEARTBEAT_WARN_INTERVAL = 60 * 1000;

function warnHeartbeatFailure(message, meta = {}) {
    const now = Date.now();
    if (now - lastHeartbeatWarnAt < HEARTBEAT_WARN_INTERVAL) return;
...
function startHeartbeat() {
    ...
    networkScheduler.setIntervalTask('heartbeat_interval', CONFIG.heartbeatInterval, () => {
        ...
        sendMsgAsync('gamepb.userpb.UserService', 'Heartbeat', body, 15000).then(({ body: replyBody }) => {
            lastHeartbeatResponse = Date.now();
            heartbeatMissCount = 0;
            try {
            ...
            warnHeartbeatFailure(`心跳失败: ${e && e.message ? e.message : '未知错误'}`, {
