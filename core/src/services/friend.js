...
const FRIEND_BAD_STATE_FILE = getDataFile('friend-bad-state.json');
const BAD_OPERATION_IDS = new Set([10003, 10004]);
...
function readBadOperationState() {
    const data = readJsonFile(FRIEND_BAD_STATE_FILE, () => ({ accounts: {} }));
    if (!data || typeof data !== 'object') return { accounts: {} };
    if (!data.accounts || typeof data.accounts !== 'object') data.accounts = {};
    return data;
}

function writeBadOperationState(data) {
    writeJsonFileAtomic(FRIEND_BAD_STATE_FILE, data && typeof data === 'object' ? data : { accounts: {} });
}

function getBadOperationAccountState(accountId = process.env.FARM_ACCOUNT_ID || '') {
    const data = readBadOperationState();
    const key = getAccountIdForBadState(accountId);
    const state = data.accounts[key];
    if (!state || state.date !== getLocalDateKey() || typeof state.unavailableOps !== 'object') {
        return { date: getLocalDateKey(), unavailableOps: {} };
    }
    return {
        date: state.date,
        unavailableOps: { ...state.unavailableOps },
        reason: state.reason || '',
        updatedAt: Number(state.updatedAt) || 0,
    };
}

function markBadOperationUnavailableToday(operationId, reason = 'unknown', accountId = process.env.FARM_ACCOUNT_ID || '') {
    const opId = toNum(operationId);
    if (!BAD_OPERATION_IDS.has(opId)) return;
    const data = readBadOperationState();
    const key = getAccountIdForBadState(accountId);
    const today = getLocalDateKey();
    const current = data.accounts[key] && data.accounts[key].date === today
        ? data.accounts[key]
        : { date: today, unavailableOps: {} };
    current.unavailableOps = current.unavailableOps && typeof current.unavailableOps === 'object'
        ? current.unavailableOps
        : {};
    current.unavailableOps[String(opId)] = true;
    current.reason = reason;
    current.updatedAt = Date.now();
    data.accounts[key] = current;
    writeBadOperationState(data);
}

function isBadOperationUnavailableToday(accountId = process.env.FARM_ACCOUNT_ID || '') {
    const state = getBadOperationAccountState(accountId);
    return BAD_OPERATION_IDS.size > 0
        && [...BAD_OPERATION_IDS].every(opId => state.unavailableOps[String(opId)] === true);
}

function isBadOperationIdUnavailableToday(operationId, accountId = process.env.FARM_ACCOUNT_ID || '') {
    const opId = toNum(operationId);
    if (!BAD_OPERATION_IDS.has(opId)) return false;
    const state = getBadOperationAccountState(accountId);
    return state.unavailableOps[String(opId)] === true;
}
...
function canOperate(opId) {
    if (isBadOperationIdUnavailableToday(opId)) return false;
    const limit = operationLimits.get(opId);
...
async function putPlantItems(friendGid, landIds, RequestType, ReplyType, method, opId) {
    let ok = 0;
    ...
    for (const landId of ids) {
        try {
            ...
            if (e.message && e.message.includes('1001046')) {
                markBadOperationUnavailableToday(opId, 'limit_error');
...
async function putInsects(friendGid, landIds) {
    return putPlantItems(friendGid, landIds, types.PutInsectsRequest, types.PutInsectsReply, 'PutInsects', 10004);
}

async function putWeeds(friendGid, landIds) {
    return putPlantItems(friendGid, landIds, types.PutWeedsRequest, types.PutWeedsReply, 'PutWeeds', 10003);
}
...
async function checkCanOperateRemote(friendGid, operationId) {
    ...
    try {
        ...
        if (BAD_OPERATION_IDS.has(toNum(operationId)) && !reply.can_operate) {
            markBadOperationUnavailableToday(operationId, 'remote_check');
        }
        return {
...
async function runBadOnceOnStartup() {
    ...
    const accountId = process.env.FARM_ACCOUNT_ID || '';
    if (isBadOperationUnavailableToday(accountId)) {
        badExecutedOnStartup = true;
        log('好友', '今日放虫放草已不可用，跳过启动时捣乱', { module: 'friend', event: '启动放虫放草跳过', reason: 'bad_unavailable_today' });
        return;
    }
