...
async function sellItems(items) {
    const payload = (items || []).filter(isSellableItem).map(toSellItem);
    if (!payload.length) {
...
function getSellBlockReason(itemOrId) {
    const id = typeof itemOrId === 'object' ? toNum(itemOrId && itemOrId.id) : toNum(itemOrId);
    if (!isFruitItemId(id)) return '不是果实';
    const info = getItemById(id) || {};
    const text = `${info.name || ''} ${info.desc || ''} ${info.effectDesc || ''}`;
    if (/不售卖|不可出售|不能出售|无法出售|禁止出售/.test(text)) return '活动期间不可出售';
    const price = Number(info.price) || 0;
    const priceId = Number(info.price_id) || 0;
    if (price <= 0 && priceId <= 0) return '无出售价格';
    return '';
}

function isSellableItem(itemOrId) {
    return getSellBlockReason(itemOrId) === '';
}
...
function getItemMutantTypes(item, id) {
    const itemMutantTypes = [];
    if (item && item.mutant_types && item.mutant_types.length > 0) {
        try {
            const allMuts = getAllMutantTypes();
            if (allMuts) {
    ...
    return Array.from(new Set(itemMutantTypes));
}

function getMutationGroupKey(item, id, itemMutantTypes) {
    const rawTypes = ((item && item.mutant_types) || [])
        .map((mt) => Number(mt) || 0)
        .filter((mt) => mt > 0)
        .sort((a, b) => a - b);
    if (rawTypes.length > 0) return `${id}:mutant:${rawTypes.join(',')}`;
    if (itemMutantTypes && itemMutantTypes.length > 0) return `${id}:mutant-name:${itemMutantTypes.slice().sort().join(',')}`;
    return `${id}:normal`;
}

function buildBagDetailFromItems(rawItems) {
    const originalItems = [];
    const merged = new Map();
    ...
    for (const it of (rawItems || [])) {
        ...
        if (id <= 0 || count <= 0) continue;
        const itemMutantTypes = getItemMutantTypes(it, id);
        const bagGroupKey = getMutationGroupKey(it, id, itemMutantTypes);
        originalItems.push({ id, count, uid, bagGroupKey });
    }
    ...
    for (const it of (rawItems || [])) {
        ...
        const sellBlockedReason = getSellBlockReason({ id });
        const sellable = sellBlockedReason === '';
        const itemMutantTypes = getItemMutantTypes(it, id);
        const bagGroupKey = getMutationGroupKey(it, id, itemMutantTypes);

        if (!merged.has(bagGroupKey)) {
            merged.set(bagGroupKey, {
                id,
                bagGroupKey,
                count: 0,
                name,
                ...
                mutantTypes: itemMutantTypes,
                isMutantItem: itemMutantTypes.length > 0,
                priceId,
        ...
        const row = merged.get(bagGroupKey);
        row.count += count;
    }
    ...
    items.sort((a, b) => {
        ...
        return String(a.bagGroupKey || '').localeCompare(String(b.bagGroupKey || ''));
    });
...
async function getBagDetail() {
    ...
    return buildBagDetailFromItems(rawItems);
}
...
module.exports = {
    ...
    __privateWarehouseFns: {
        buildBagDetailFromItems,
        getItemMutantTypes,
        getMutationGroupKey,
    },
};
