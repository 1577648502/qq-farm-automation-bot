...
const { getPlantNameBySeedId, getPlantName, getPlantExp, formatGrowTime, getPlantGrowTime, getAllSeeds, getPlantById, getPlantBySeedId, getSeedImageBySeedId, resolveMutationInfo, getAllMutantTypes } = require('../config/gameConfig');
const { isAutomationOn, getPreferredSeed, getAutomation, getPlantingStrategy, getBagSeedPriority, getBagSeedFallbackStrategy, getPlantSeedExclude, getFertilizerBuyOrganicCount, getFertilizerBuyOrganicThresholdHours, getFertilizerBuyNormalCount, getFertilizerBuyNormalThresholdHours, getFertilizerBuyCheckIntervalMinutes } = require('../models/store');
...
function getDisplayLandContext(land, landsMap) {
    ...
    const ownSlaveIds = hasPlantData(land) ? getSlaveLandIds(land) : [];
    const ownOccupiedLandIds = [selfId, ...ownSlaveIds].filter(Boolean);
    return {
        sourceLand: land,
        occupiedByMaster: false,
        masterLandId: selfId,
        occupiedLandIds: ownOccupiedLandIds.length > 0 ? ownOccupiedLandIds : [selfId].filter(Boolean),
    };
}
...
const ALL_FERTILIZER_LAND_TYPES = ['amethyst', 'gold', 'black', 'red', 'normal'];
...
async function plantSeeds(seedId, landIds, options = {}) {
    ...
    for (const rawLandId of landIds) {
        ...
        try {
            // 2x2 作物只提交主地块，并让服务端自动选择相邻的 3 个副地块。
            // 未设置 auto_slave 时，服务端会返回 1001052「选择错误的地块格子」。
            const body = encodePlantRequest(seedId, [landId], autoSlave);
...
function getPlantSeedExcludeSet() {
    return new Set((getPlantSeedExclude() || []).map(id => Number(id)).filter(id => id > 0));
}

function isPlantSeedExcluded(seedId, excludeSet = getPlantSeedExcludeSet()) {
    return excludeSet.has(Number(seedId) || 0);
}
...
function sortBagSeedsForPlanting(bagSeeds, priorityList) {
    ...
    return [...(Array.isArray(bagSeeds) ? bagSeeds : [])].sort((a, b) => {
        ...
        // 未手动排序时，大作物优先消耗，避免先种满 1x1 后再也凑不出 2x2 空地。
        const aSize = Math.max(1, Number(a.plantSize || 1));
        const bSize = Math.max(1, Number(b.plantSize || 1));
...
function getBagSeedPlantCapacity(seed, availableLandCount) {
    const plantSize = Math.max(1, Number(seed && seed.plantSize || 1));
    const landFootprint = plantSize * plantSize;
    return Math.min(
        Math.max(0, Number(seed && seed.count || 0)),
        Math.floor(Math.max(0, Number(availableLandCount) || 0) / landFootprint),
    );
}
...
async function plantFromBagSeeds(landsToPlant) {
    ...
    const excludeSet = getPlantSeedExcludeSet();
    const bagSeedsWithCount = allBagSeeds.filter(seed => Number(seed && seed.count) > 0);
    const usableSeeds = sortBagSeedsForPlanting(
        bagSeedsWithCount.filter(seed => !isPlantSeedExcluded(seed && seed.seedId, excludeSet)),
        getBagSeedPriority(),
    );

    if (usableSeeds.length === 0) {
        const hasAnyBagSeed = bagSeedsWithCount.length > 0;
        const hasExcludedBagSeed = bagSeedsWithCount.some(seed => isPlantSeedExcluded(seed && seed.seedId, excludeSet));
        log('种植', hasAnyBagSeed
            ? (hasExcludedBagSeed ? '背包种子均已排外，准备按第二优先策略补种' : '背包中没有可用种子，准备按第二优先策略补种')
            : '背包种子已用完，准备按第二优先策略补种', {
            module: 'farm',
            event: '种植种子',
    ...
    for (const seed of usableSeeds) {
        ...
        const plantSize = Math.max(1, Number(seed.plantSize || 1));
        const maxPlantCount = getBagSeedPlantCapacity(seed, remainingLandIds.length);
        ...
        if (result.planted > 0) {
            ...
            usedSeedLogs.push(`${seed.name}${plantSize > 1 ? `(${plantSize}x${plantSize})` : ''}x${result.planted}`);
        }
        ...
        if (result.planted < maxPlantCount && remainingLandIds.length > 0) {
            ...
            logWarn('种植', `背包种子 ${seed.name} 实际种植 ${result.planted}/${maxPlantCount}，${isPureLayoutFailure ? '没有满足种植条件的土地，本轮执行第二优先策略' : '为避免误购商店种子，本轮不执行第二优先策略'}`, {
                seedId: seed.seedId,
...
async function findBestSeed(overrideStrategy) {
    ...
    if (available.length === 0) {
        logWarn('商店', excludedAvailableCount > 0 ? '没有可购买且未排外的种子' : '没有可购买的种子');
        return null;
    }
...
async function getLandsDetail() {
    try {
        ...
        for (const land of landsReply.lands) {
            ...
            let isMutant = mutationInfo ? mutationInfo.isMutant : false;
            const isGolden = mutationInfo ? mutationInfo.isGolden : isGoldenPlant(plantId);
            let mutationType = null;
            let mutationQuality = null;
            let mutationKey = null;

            if (mutationInfo && mutationInfo.mutationType) {
                        ...
                        if (mutantTypes) {
                            ...
                            for (var ki = 0; ki < keys.length; ki++) {
                                ...
                                if (mcfg && (!mcfg.qualityRank || mcfg.qualityRank > 2)) {
                                    ...
                                    mutationType = mt;
                                    mutationQuality = mcfg.quality || '?';
                                    mutationKey = mt;
                                    break;
                                }
                            }
                            if (mutationType) break;
                        }
                    }
                }
            }
                            ...
                            if (configIds.length > 0) {
                                ...
                                if (mi && mi.mutationType) {
                                    mutantCrops.push({
                                        ...
                                        mutationType: mi.mutationType,
                                        mutationQuality: mi.mutationQuality,
                                        mutationKey: mi.mutationKey || mi.mutationType,
                                    });
                                }
                            }
                        }
