...
const DEFAULT_FERTILIZER_LAND_TYPES = ['amethyst', 'gold', 'black', 'red', 'normal'];
...
function cloneAccountConfig(base = DEFAULT_ACCOUNT_CONFIG) {
    ...
    // 种子排外列表
    const rawPlantSeedExclude = Array.isArray(base.plantSeedExclude) ? base.plantSeedExclude : [];
...
function applyConfigSnapshot(snapshot, options = {}) {
    ...
    // 种子排外列表
    if (Array.isArray(cfg.plantSeedExclude)) {
        next.plantSeedExclude = cfg.plantSeedExclude.map(Number).filter(n => Number.isFinite(n) && n > 0);
    }
...
// ============ 种子排外列表 ============
function getPlantSeedExclude(accountId) {
    return [...(getAccountConfigSnapshot(accountId).plantSeedExclude || [])];
}

function setPlantSeedExclude(accountId, list) {
    const current = getAccountConfigSnapshot(accountId);
    const next = normalizeAccountConfig(current, accountFallbackConfig);
    next.plantSeedExclude = Array.isArray(list) ? list.map(Number).filter(n => Number.isFinite(n) && n > 0) : [];
    setAccountConfigSnapshot(accountId, next);
    return [...next.plantSeedExclude];
}
...
function addOrUpdateAccount(acc) {
    ...
    // 备注里填写 QQ 号时以备注为准；QQ 号可能是 8-12 位。
    const normalizedUin = normalizeQQUin(acc.name) || (acc.uin ?? acc.wxid ?? acc.qq ?? '');
    if (acc.id) {
        const idx = data.accounts.findIndex(a => a.id === acc.id);
        if (idx >= 0) {
            data.accounts[idx] = {
                ...data.accounts[idx],
                ...acc, name: acc.name !== undefined ? acc.name : data.accounts[idx].name, updatedAt: Date.now() };
